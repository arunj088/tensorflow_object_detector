...
def animate(xy_loc, fig, ax):
    ...
    if not vehicle_pos[0,0] < 0:
        ax.add_patch(patches.Rectangle(((vehicle_pos[0,1]-0.1),vehicle_pos[0,0]),0.20,.20, fill=False, edgecolor='red', alpha =2))
    ...
    if 0 < vehicle_pos[0,0] <= 7.5:
        ...
        #         lane_loc[i,1] = lane_loc[i,1]+.05 # left lane
        #     if (lane_loc[i,0]>=vehicle_pos[0,0]) and ((lane_loc[i,0]) <= (vehicle_pos[0,0] + car_length)):
        #         lane_loc[i,3] = lane_loc[i,3] + .05 # right lane
